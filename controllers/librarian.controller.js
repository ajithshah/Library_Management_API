...
exports.bulkDelete = async (req, res) => {
  if (req.role === "librarian" || req.role === "admin") {
    try {
      await Books.deleteMany({ _id: { $in: req.body.delete_id } });
      res.status(200).send("Deleted id's successfully");
    } catch (err) {
      res.status(500).send("Error" + err);
    }
  } else res.send("Access denied");
...
exports.fineEntry = async (req, res) => {
  ...
  if (req.role === "librarian" || req.role === "admin") {
    ...
    if (returnedDate > dueDate) {
      ...
      res.status(200).send(`You have to pay fine amount Rs. ${fine} .`);
